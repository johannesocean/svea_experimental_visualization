...
cfig = Config(
    data_file_name='phyche_archive_ferrybox_2018-2022.feather'
)

PARAMETERS = ['FLUO_FB', 'FLUO_CTD', 'DEPH', 'SEA']
        ...
        for dep_itv in ['3-7']:
            for sun_angle in ('', '>30', '<-30'):
                for item in fig_types:
                    boolean = df['FLUO_FB'].notna() & df['FLUO_CTD'].notna()
                    ...
                    if sun_angle == '<-30':
                        boolean = boolean & (df['SUN_ANGLE'] < -30)
                    if dep_itv == '3-7':
                        boolean = boolean & (df['DEPH'] >= 3) & (df['DEPH'] <= 7)
                    df_patch = df.loc[boolean, PARAMETERS].reset_index(drop=True)
                    if df_patch.__len__() < 2:
                        continue
                    if log_bool:
                        df_patch['FLUO_FB'] = np.log10(df_patch['FLUO_FB'])
                        df_patch['FLUO_CTD'] = np.log10(df_patch['FLUO_CTD'])

                    fig = plt.figure()
                    ax = fig.add_subplot(111)
                    r, p = stats.pearsonr(df_patch['FLUO_FB'], df_patch['FLUO_CTD'])
                    p = '< 0.05' if p < 0.05 else str(p)

                    linregs, eq, stat_values = linreg_eq_data(
                        df_patch['FLUO_FB'], df_patch['FLUO_CTD'])
                    stats_text = 'R={:.2f}, p={}\n{}'.format(r, p, eq)
                    ...
                    max_of_max = max([df_patch['FLUO_FB'].max(), df_patch['FLUO_CTD'].max()])
                    min_of_min = min([df_patch['FLUO_FB'].min(), df_patch['FLUO_CTD'].min()])
                    ...
                    ax.set_xlabel('FerryBox chlorophyll fluorescence (au)')
