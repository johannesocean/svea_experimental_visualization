...
class PhyCheArchive:
    ...
    def __init__(self, archive_path=None):
        archive_path = archive_path or r'C:\PhysicalChemical\2020\SHARK_PhysicalChemical_2020_BAS_SMHI\processed_data\data.txt'
        df = text_reader(
            'pandas',
            archive_path,
            encoding='cp1252',
        ...
        self.df['timestamp'] = self.df[['SDATE', 'STIME']].apply(lambda x: pd.Timestamp(' '.join(x)), axis=1)
